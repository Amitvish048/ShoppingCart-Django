...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            #messages.success(request, 'Thank you for registertion activation link has been send to your e-mail address[vishwakarma1708@gmail.com]')
            ...
            try:
                ...
                if is_cart_item_exists:
                    ...
                    for pr in product_variation:
                        if pr in ex_val_list:
                            index = ex_val_list.index(pr)
                            item_id = id[index]
...
def resetPassword(request):
    if request.method == 'POST':
        ...
        if password == confirm_password:
            uid = request.session.get('uid')   # ✅ use string key
            if uid is None:
